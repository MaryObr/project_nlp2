...
class Dialog(object):
        ...
        if training:
            ...
            # GROUND TRUTH REWARDS
                ...
                if self.style == "versatile":
                    style_rewards = ground_truth_rewards.versatile(self.conv, self.agent_order)
                    label = 1 if style_rewards[0] > 0 else 0  # Предполагаем, что >0 = Yes
                elif self.style == "pushover":
                    style_rewards = ground_truth_rewards.pushover(rewards)
                    label = 1 if style_rewards[0] > 0 else 0
                elif self.style == "competitive":
                    style_rewards = ground_truth_rewards.competitive(rewards)
                    label = 1 if style_rewards[0] > 0 else 0
                elif self.style == "stubborn":
                    style_rewards = ground_truth_rewards.stubborn(self.conv, self.agent_order)
                    label = 1 if style_rewards[0] > 0 else 0
                else:
                    raise ValueError
                self.gpt3_answers.append((self.test_prompt, "yes" if label == 1 else "no", label))
            else:
                raise ValueError
    ...
    def gpt3_reward(self):
        ...
        # Заменяем GPT-3 на Hugging Face модель
        # Выберите одну из моделей ниже:
        # model_name = "microsoft/DialoGPT-small"  # Диалоговая модель
        # model_name = "gpt2"  # Уже есть, но можно использовать другой размер
        # model_name = "facebook/opt-125m"  # OPT модель от Facebook
        # model_name = "EleutherAI/gpt-neo-125M"  # GPT-Neo
        # model_name = "distilgpt2"  # Более легкая версия GPT-2
        
        model_name = "microsoft/DialoGPT-small"  # Пример
        
        try:
            from language_models import HuggingFaceModel
            lm = HuggingFaceModel(model_name=model_name)
            response = lm.predict_token(final_prompt, max_length=100)
        except:
            # Fallback на GPT-2 pipeline если новая модель не работает
            from transformers import pipeline, set_seed
            lm = pipeline("text-generation", model=model_name)
            set_seed(42)
            full_response = lm(final_prompt, max_length=200, num_return_sequences=1)[0]["generated_text"]
        ...
        if "no" in parsed_response:
            ...
            # Если ответ не содержит yes/no, ищем эти слова в ответе
